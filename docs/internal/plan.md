...
status:
  ...
  pest_suite: 329 passing / 0 skipped / 634 assertions
  ...
  p1_remaining: 0
  p2_remaining: 3 (P2.1, P2.2, P2.8; P2.4 absorbed into P1.12; P2.5 done as Phase E; P2.6 absorbed into Phase D verification gate)
...
| 2 | [Manual testing remaining](#2-manual-testing-remaining) | active — 78/79 PASS; T1.2 + TX.2 + T9.7 now covered by Pest (E6); T12.6 + T13.6 + T13.11 require Enterprise/browser/SR gates |
...
- **Phase E closed 2026-05-02.** 18 commits (E1 bootstrap + HibpClient extraction; E2 pure validators; E3 DB-touching services + factories; E4 HIBP backoff + StrengthService + SettingsModel alias; E5 ValidationController + destroyOtherSessions + PasswordWidgetTag; E6 migration replay + multi-site propagation + FK CASCADE) plus a `fix(hibp)` log-level downgrade and a `docs(ideas)` cleanup-candidates capture. **Pest suite: 329 passing / 0 skipped / 634 assertions.** ECS clean, PHPStan clean (3-entry baseline unchanged from E1). All three deferred manual tests (T1.2, TX.2, T9.7) now covered by Pest. The C2 bug-fix sweep is now pinned by 32 of E4+E5's tests. Phase E session log: `history/progress-phase-e.md`.
...
- **Next: Phase D (User index integration).** Build order swap (E before D, signed off 2026-05-02) means D now lands on top of a 329-test regression net. P2.1 (table attributes) + P2.2 (admin password change action) + the half-built `_users/password-security.twig` user-edit tab. P2.6 (`allowAdminChanges` read-only verification) absorbed as a quality gate, not a separate item.
...
- **P2 backlog: 3 items remaining** — P2.1, P2.2 (Phase D), P2.8 (Phase F, Phase G build plan draft). P2.4 absorbed into P1.12. **P2.5 closed as Phase E (2026-05-02).** P2.6 absorbed into Phase D verification gate.
...
| **D** | **User index integration (P2.1, P2.2; absorbs P2.6 `allowAdminChanges` verification gate)** | **next** |
| F | Polish + draft Phase G build plan (P2.8) — P2.4 absorbed into P1.12; P2.6 absorbed into D | pending |
| G | Enterprise (Phase 10, 11, 12) — driven by P2.8 build plan | blocked on D + F |
...
- D cannot start until E is complete (Phase D extends the codebase with new CP surfaces; Pest must cover the existing C2 surface before new code lands on top). **E now done — D is unblocked.**
