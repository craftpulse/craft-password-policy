...
status:
  ...
  pest_suite: 513 passing / 0 skipped / 1049 assertions
  ...
  p1_remaining: 0
  p2_remaining: 1 (P2.8; P2.1 + P2.2 done as Phase D; P2.4 absorbed into P1.12; P2.5 done as Phase E; P2.6 absorbed into Phase D verification gate)
...
- **Next: Phase F (polish + draft Phase G build plan, P2.8).** Phase D + E are the regression and feature net Phase G can build on. P2.8 deliverables: lock hash-chained audit row format (canonical JSON shape, `previousHash` column), independent verifier CLI exit codes + chain-walk semantics, SIEM forwarders, webhook HMAC scheme, `AlertCooldownService` scope, streaming audit-log export pattern. Use `internal/history/phase-c2-build-plan.md` as the structural template.
...
| **F** | **Polish + draft Phase G build plan (P2.8) — P2.4 absorbed into P1.12; P2.6 absorbed into D** | **next** |
...
- F cannot start until D is complete (P2.8 Phase G build plan benefits from the user-index work being in-hand for cross-references). **D now done — F is unblocked.**
