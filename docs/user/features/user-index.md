...
When multiple states apply, the composite `passwordpolicy_status` badge shows the most-severe — order:
...
| `PolicyDriftConditionRule` | Pro + Craft Team or higher | Filter users whose stored policy snapshot diverges from the current resolver output — flags users who haven't re-validated since a policy change. |
...
| `ChangeUserPassword` | All editions | Single-user modal — admin types a new password, the action validates it against the resolved policy, then writes the new hash. The audit context propagates as `AdminChange` with `changedByUserId` set to the operator. Bulk-change-with-same-password is a security anti-pattern; the modal is single-user only. |
