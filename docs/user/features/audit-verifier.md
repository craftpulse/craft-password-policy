...
Exit code `2`. A row's data is corrupt — JSON column won't parse, datetime column has a non-canonical value, etc. The chain integrity is undetermined for that row.
