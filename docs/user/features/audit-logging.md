...
- The `id` column is **excluded** because auto-increment isn't deterministic across database restores. Chain order is established by `dateCreated` + insertion order.
...
- [SIEM forwarders](./siem-forwarders.md): Syslog over TLS to rsyslog, syslog-ng, Graylog, QRadar, and any RFC 5424 receiver, or a JSON POST over HTTPS.
