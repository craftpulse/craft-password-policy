...
- **CP templates** check `getIsPro()` / `getIsEnterprise()` to render or omit edition-gated affordances. Lower editions don't render higher-edition surfaces at all: a Lite admin never sees the Pro-only Group Policies / Compliance Presets / Validators (advanced) settings, and a Pro admin never sees the Enterprise-only fields. Whole-screen higher-edition settings sections are also omitted from the settings sidebar, and their controller denies direct URL access (403) as defense in depth. Mixed pages (Configuration, Password Rules, Password History, Validators, Audit Logging) render on every edition and simply omit their higher-edition fields.
