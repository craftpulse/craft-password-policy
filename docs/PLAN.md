...
status:
  ...
  p1_remaining: 0 (P1.12 layer 4b CP-side strength deferred to a separate session before tag)
...
| ~~P1.12~~ | ~~Pro front-end Twig surface~~ — done 2026-05-01 (Layer 4b deferred) | Image-optimize-style fluent render builders shipped under `craft.passwordpolicy.*` (`passwordField`, `requirementList`, `strengthMeter`, `requirementsHint`, `passwordWidget`, `loginForm`, `passwordChangeForm`, `passwordResetForm`). Data accessors `requirements()` / `requirementsText()` / `requirementRules()` with optional `groups` for anonymous group preview. Vanilla-JS client asset (`PasswordPolicyClientAsset`) auto-registered by any builder with `liveValidation: true` — no Vite assumption on consumer side. Strength engine A (baseline rule-counting × length-tier) ships free; engine B (zxcvbn-php opt-in) adds score 0-4 + suggestions + crackTime. New `Front\PasswordChangeController` for logged-in change. **Layer 4b (CP-side strength meter replacement on Pro CP requests) deferred** — DOM stability research + Garnish-style work warrants a focused session before tag. T13.1–T13.12 in TESTING.md. Full reference at `docs/10-frontend-twig-surface.md`. |
...
| C2 | P1 expansion — Pro front-end surface bundle (P1.14 RegistrationService → P1.13 HIBP-on-login → P1.12 front-end Twig render builders + JS asset + strength engine A+B → P1.15 events catalog). | done 2026-05-01 (Layer 4b CP-side strength replacement deferred to a separate session before 5.2.0 tag) |
