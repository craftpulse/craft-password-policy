...
- Twig variables — `craft.passwordPolicy.passwordStatus()`, `daysUntilExpiry()`, `isExpiring(N)`, `activeSessionCount()` (also available under the legacy `craft.passwordpolicy` lowercase handle for backward compatibility with 5.1.1 consumers — both forms work permanently)
...
- Front-end Twig surface (Pro front-end, Lite-degrades-to-global) — image-optimize-style fluent render builders on `craft.passwordPolicy.*` for building user-facing login/registration/reset/change-password pages. Field renderers: `passwordField()`, `requirementList()`, `strengthMeter()`, `requirementsHint()`, `passwordWidget()` (composite). Form renderers: `loginForm()` (POSTs to `users/login`), `passwordChangeForm()` (logged-in user, POSTs to `password-policy/front/password-change/save`), `passwordResetForm()` (token-based, POSTs to `users/set-password`). Each builder accepts chained setters AND an equivalent config-array constructor; `*Attrs()` setters per element merge into the rendered markup with empty values omitted. a11y baked in: live region, `aria-describedby` linking input to requirements list, `aria-invalid` toggled on validation state, `aria-busy` during AJAX, screen-reader announcements for pass/fail transitions, `role="progressbar"` on the strength meter with `aria-valuemin/max/now`. Show/hide toggle defaults on with FA-style eye/eye-slash inline SVG (MIT path data, no asset dep) and flipping `aria-label`.
- Data accessors on `craft.passwordPolicy.*` — `requirements({groups: [...]})` returns the resolved policy as a flat typed array (Lite returns global; Pro resolves per-group; anonymous group preview supported via the `groups` parameter); `requirementsText()` returns a human-readable single-sentence summary for hint copy; `requirementRules()` returns `[{key, label, met}]` for devs rendering bespoke checklists.
- Pre-built JavaScript client in `src/web/assets/passwordpolicyclient/password-policy.js` — vanilla, framework-free, ~5KB unminified. Auto-registered by any builder when `liveValidation: true`. Consumers don't need Vite, esbuild, or any build step. Attaches to `[data-pp-validate]`, debounces 250ms, POSTs to `password-policy/validation/validate`, toggles `pp-pass`/`pp-fail` on `[data-pp-requirement]` items, updates `pp-strength-{label}` on `[data-pp-strength]` wrappers, gates `[data-pp-submit-gate]` button enable/disable. Includes show/hide toggle binding and live-region announcements.
- Server-side strength engine (`bjeavons/zxcvbn-php`, MIT) — analyses passwords against common patterns, dictionaries, keyboard sequences, and (when context is supplied) the user's own username/email. Returns a 0-4 score, crack-time estimate, suggestions array, and optional warning. Same engine drives the CP strength meter (`showStrengthIndicator`, every edition) and the front-end Twig render builders (`craft.passwordPolicy.passwordField()` / `passwordWidget()` / `strengthMeter()` etc., Pro). Blocklist hit forces `label = 'weak'` + `score = 0` regardless of zxcvbn's natural reading — without this override a blocklisted long+complex password reads as "excellent" while the back-end correctly rejects it. The narrow override clamps `label` + `score` only; `crackTime`, `suggestions`, and `warning` carry through unchanged so users still see the dictionary breakdown.
- New `Front\PasswordChangeController` for logged-in-user password changes from consumer site templates. Validates current password via `User::authenticate()` (already understands suspended/locked states), validates new password via the resolved per-group/global policy, saves through `Craft::$app->elements->saveElement()`. POST target for `passwordChangeForm()`.
...
- New `StrengthService` — registered in `ServicesTrait` as `strength`. Wraps zxcvbn-php into the plugin's stable `{engine, label, score, crackTime, suggestions, warning}` response shape; `compute()` is a thin pass-through to `analyzeZxcvbn()` so a future engine swap doesn't have to renegotiate call sites.
...
- CP password strength indicator now consumes the same AJAX `password-policy/validation/validate` endpoint as the front-end builders — single strength engine across CP and consumer surfaces. Blocklist hits and per-group policy resolution surface in the CP indicator without the previously-required client-side duplication. Selector generalized from `#newPassword` to `input[type="password"][autocomplete="new-password"]:not([data-pp-no-strength])` so the indicator now also attaches on the installer + set-password screens. Dropped `@zxcvbn-ts/core`, `@zxcvbn-ts/language-common`, and `@zxcvbn-ts/language-en` from the buildchain in favor of `bjeavons/zxcvbn-php` (server-side, already required) — the JS bundle dropped from ~1.65 MB to ~2.2 KB.
...
- Strength meter now respects `blocklistHit` — a blocklisted word reads as "weak" / score 0 (previously zxcvbn-php returned its own score, leaving the meter green while the rule list correctly rejected the password)
