...
- `password-policy/retention/force-reset-passwords`: bulk force-reset (CSRF-protected; user-id list).
...
- **Force password reset is Pro on every surface.** The `ForcePasswordReset` bulk element action and the "Force password reset" user-edit action-menu item were already Pro; the Actions pane on the user-edit **Password Security** screen was not, so a Lite operator could still force a reset from there. The pane now renders on Pro+ only (absent below Pro, per the hide-not-badge doctrine, driven by a `showForceReset` flag that defaults closed so a missing variable fails safe), the `pp:force-reset-passwords` permission registers on Pro+ only, and `UserSecurityController::actionForceReset()` gates on edition before permission so a Lite POST answers **404** rather than 403. The Password Security screen itself stays universal on every edition, it just omits the pane. Existing `pp:force-reset-passwords` grants survive a downgrade untouched (Craft never prunes `userpermissions` rows for unregistered names) and take effect again on upgrade, so the Retention utility's bulk force-reset keeps working on Lite for admins and for anyone already holding the grant.
