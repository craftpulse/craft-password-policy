...
- Strength engine A (baseline) — rule-counting × length-tier label (`weak/fair/strong/excellent`), no dependencies. Length tiers: <8 / 8-11 / 12-15 / 16+. Blocklist hit forces `weak` regardless of length so users see consistent feedback when the back-end will reject the password.
- Strength engine B (zxcvbn-php Pro opt-in) — opt in via `useZxcvbnStrength: true` (default off, Pro). Adds `bjeavons/zxcvbn-php` composer dep (~200KB, MIT). Returns score 0-4, crack-time string, suggestions array, warning. Same response shape — consumers reading `strength.label` work in either mode.
