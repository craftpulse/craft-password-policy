...
- Plugin editions: Lite (default), Pro, Enterprise — gated via project config.
...
- Server-side strength engine — `bjeavons/zxcvbn-php` (MIT). Analyses passwords against patterns, dictionaries, keyboard sequences, and (when context is supplied) the user's own username/email. Returns 0–4 score, crack-time estimate, suggestions, optional warning.
