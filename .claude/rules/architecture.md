...
├── exceptions/                     # custom exception classes (EditionRequiredException, …)
├── jobs/                           # queue jobs
...
- **Edition-gate throws — layered convention**:
  - **Service-layer + Twig-variable-layer gates** throw `craftpulse\passwordpolicy\exceptions\EditionRequiredException` (extends `\RuntimeException`). Used by `PasswordPolicyVariable::_assertProForBuilders()` and `NotificationService::sendBreachDetected()` / `sendNewDeviceAlert()`. The dedicated subclass lets integrators catch the gate explicitly; generic `catch (\RuntimeException)` handlers still work via the inheritance chain. Twig surfaces the exception in dev mode and renders the friendly error template in production.
  - **HTTP controller gates** throw `yii\web\ForbiddenHttpException` (not `EditionRequiredException`) — Craft's web layer expects an HttpException to render a proper 403 response with the correct status code, headers, and error template. Throwers include `NotificationTemplateController`, `SettingsController::actionApplyPreset`, `AuditExportController`, `SiemForwarderController`, `WebhookEndpointController`, `NotificationActivityController`, `PolicyController`, `ReportController` (non-exhaustive — grep `throw new ForbiddenHttpException` against `src/controllers/` to enumerate current sites).
  - **Queue jobs + console controllers** skip gracefully — log `Craft::warning` and return early (jobs) or write to stderr and return non-zero exit code (console). No throw. The async / CLI surface can't tolerate exception-driven control flow without surfacing user-visible problems.
- **Front-end render builders** (`craft.passwordPolicy.passwordField()` / `passwordWidget()` / `loginForm()` / `passwordChangeForm()` / `passwordResetForm()` / `requirementList()` / `strengthMeter()` / `requirementsHint()`): Pro-gated via `PasswordPolicyVariable::_assertProForBuilders()` per the edition-gate convention above. Pro-only is a deliberate upgrade-incentive choice (the friendly consumer-form surface is a load-bearing reason operators upgrade); Lite consumers roll their own markup against the universal **data accessors** (`requirements()`, `requirementsText()`, `requirementRules()`). Per-group policy resolution on top of the builders gates separately inside `PolicyResolverService` (Pro + `enablePerGroupPolicies = true`).
