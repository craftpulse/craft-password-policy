...
- **Edition-gate throws — layered convention**:
  ...
  - **HTTP controller gates** throw `yii\web\ForbiddenHttpException` (not `EditionRequiredException`) — Craft's web layer expects an HttpException to render a proper 403 response with the correct status code, headers, and error template. Used by `NotificationTemplateController`, `SettingsController::actionApplyPreset`, `AuditExportController`, `SiemForwarderController`, `WebhookEndpointController`, `NotificationActivityController`.
